/**
 * GRAPH BRAIN V1's OWN observation types.
 *
 * EXTRACTED verbatim from production `pursuer-v2/brain/observation.ts` at
 * commit d7a8115, with only the two import paths changed. The lab's own
 * `contract/observation.ts` is the general interface every Brain sees; these
 * are the shapes THIS Brain was written against, and keeping them unchanged
 * is what lets the baseline reproduce production behaviour exactly. The
 * adapter in `index.ts` translates between the two.
 *
 * ── the original production note ──
...
import type { SearchCursorState } from '../../graph/frontierSearch';
