/**
 * EXTRACTED from production `pursuer-v2/brain/graphBrainV1.ts` at commit
 * d7a8115. ADAPTED in exactly one respect: the six constants that were module
 * literals are now fields on a `GraphV2Tuning` object threaded through
 * `updateBrain`, with `GRAPH_V2_BASELINE_TUNING` holding the production
 * values. Brain A passes that frozen object and is therefore behaviourally
 * identical to production; Brain B passes a tunable one. Nothing else in the
 * decision logic is changed, and `graphV2Parity.test.ts` fails if it ever is.
 *
 * The long derivations below are production's own and are kept because they
 * are the record of WHY each window is the length it is. Note that all four
 * are counted in TICKS: in this lab a tick is a fixed simulation step, so
 * their wall-clock length no longer depends on the display. See
 * `sim/timebase.ts`.
 *
...
import type {
  ...
  CommitmentEndReason,
} from './brainObservation';
import { nextSearchTarget } from '../../graph/frontierSearch';
...
/**
 * The six quantities Brain B may move. Every one of them was DERIVED from a
 * measured oscillation rather than chosen — the comments above each constant
 * are that derivation — so moving one is an experiment, not a correction.
 */
export interface GraphV2Tuning {
...
/** The production values. Brain A runs on exactly this object. */
export const GRAPH_V2_BASELINE_TUNING: GraphV2Tuning = Object.freeze({
...
function upsertFragments(
  ...
  return all.slice(0, maxRememberedFragments);
}
...
export function updateBrain(
  ...
  const rememberedFragments = upsertFragments(prev.rememberedFragments, obs.sensedTrailFragments, obs.nowMs, tuning.maxRememberedFragments);
  ...
  const directLossConfirmed = unsensedRunTicks >= tuning.lossConfirmationTicks;
  const directStablyReacquired = sensedRunTicks >= tuning.acquireConfirmationTicks;
  ...
  const leadStablyActionable = bestIsActionable
    && (bestNeverInvestigated || actionableLeadRunTicks >= tuning.leadPreemptionConfirmationTicks);
  ...
  if (commitment) {
    if (commitment.mode === 'VISIBLE_PURSUIT') {
      ...
      if (!committedFragment) commitmentEndReason = 'TARGET_INVALIDATED';
        ...
        if (trailExhaustionTicks >= tuning.trailExhaustionConfirmationTicks) {
      ...
      else if (distance(obs.pursuerPosition, { x: node.x, y: node.y }) < tuning.arrivalEpsilon
