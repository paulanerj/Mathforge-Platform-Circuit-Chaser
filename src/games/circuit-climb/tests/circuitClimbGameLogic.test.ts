...
describe('Circuit Climb Model & Math Logic', () => {
  ...
  describe('FEATURE-01: Powered Platform Activation State', () => {
    it('A, B & C. Correct arrival sets powered to true and retains internal value, wrong arrival does not', () => {
      // Simulate platform arrivals
      const correctPlatform = { value: 7, correct: true, powered: false, dead: false };
      const wrongPlatform = { value: 3, correct: false, powered: false, dead: false };
      
      // Correct arrival
      if (correctPlatform.correct) {
        correctPlatform.powered = true;
      } else {
        correctPlatform.dead = true;
      }

      // Wrong arrival
      if (wrongPlatform.correct) {
        wrongPlatform.powered = true;
      } else {
        wrongPlatform.dead = true;
      }

      // A: correct arrival sets powered
      expect(correctPlatform.powered).toBe(true);
      // B: internal value is preserved
      expect(correctPlatform.value).toBe(7);
      
      // C: wrong arrival does NOT set powered
      expect(wrongPlatform.powered).toBe(false);
      expect(wrongPlatform.dead).toBe(true);
      expect(wrongPlatform.value).toBe(3);
    });

    it('D & E. Restart produces normal unpowered future platforms, row-0 base exception is respected conceptually', () => {
      const rows: any[] = [];
      rows.push(simulateMakeRow(0, rows));
      rows.push(simulateMakeRow(1, rows));
      
      // By default in row generation
      rows[1].platforms.forEach((p: any) => {
        expect(p.powered).toBeFalsy();
        expect(p.dead).toBe(false);
        expect(typeof p.value).toBe('number');
      });

      // row 0 is traditionally powered conceptually but its value is handled outside loop logic
      // In drawing logic, (platform.row > 0 && platform.powered) dictates activation visual
      const isActivationVisual = (row: number, powered: boolean) => row > 0 && powered;
      
      expect(isActivationVisual(0, true)).toBe(false); // Base platform exception
      expect(isActivationVisual(1, true)).toBe(true); // Normal correct landing
    });
  });
});
